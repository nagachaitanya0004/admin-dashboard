...
export default function UsersPage() {
  ...
  const columns = [
    ...
    { key: 'revenue', label: 'Revenue', sortable: true, render: (row) => <span className="font-medium text-white">{row.revenue}</span> },
    ...
    <div className="space-y-6 animate-fade-in" style={{ animationDuration: '0.4s' }}>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Manage team members and customers.</p>
        </div>
      ...
      <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl overflow-hidden shadow-sm">
        <DataTable 
          ...
          onSort={handleSort}
          onResetFilters={() => { setFilter('role', 'All'); setFilter('status', 'All'); setSearchTerm(''); }}
          showPagination={true}
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-slate-400 mb-2">{modal.data.email}</p>
                <div className="flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-4">
                <p className="text-xs font-medium text-slate-500 mb-1 flex items-center gap-1.5"><DollarSign className="w-3.5 h-3.5" /> Total Revenue</p>
                <p className="text-lg font-bold text-white">{modal.data.revenue}</p>
              </div>
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-4">
                <p className="text-xs font-medium text-slate-500 mb-1 flex items-center gap-1.5"><Activity className="w-3.5 h-3.5" /> Total Orders</p>
                <p className="text-lg font-bold text-white">{modal.data.orders}</p>
              </div>
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-4">
                <p className="text-xs font-medium text-slate-500 mb-1 flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> Member Since</p>
                <p className="text-lg font-bold text-white">{modal.data.joinDate}</p>
              </div>
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-xl p-4">
                <p className="text-xs font-medium text-slate-500 mb-1 flex items-center gap-1.5"><Activity className="w-3.5 h-3.5" /> Last Active</p>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="flex gap-3 items-start">
                    <div className="w-8 h-8 rounded-full bg-white/[0.03] flex items-center justify-center border border-white/[0.05] shrink-0">
                      <act.icon className="w-4 h-4 text-slate-400" />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-slate-300">{act.title}</p>
            ...
            <div className="flex items-center gap-3 pt-4">
              ...
              <button className="flex-1 py-2.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 rounded-xl text-sm font-medium text-rose-400 transition-colors">
